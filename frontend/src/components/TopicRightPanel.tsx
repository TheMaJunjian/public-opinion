          ...
          <span data-guide-contribution-consumption="true" style={{ fontSize: 11, color: p.totalConsumption.total > p.availablePoints ? "#f87171" : "#f59e0b" }}>
            ...
            <span style={{ color: "#888" }}>
              （{[p.totalConsumption.hasText ? `文本 ${p.totalConsumption.textStake}` : null,
                ...
                (p.totalConsumption as any).reclassificationJoinCount > 0 ? `撤销加入 ${(p.totalConsumption as any).reclassificationJoinStakeTotal + (p.totalConsumption as any).reclassificationStakeTotal + ((p.totalConsumption as any).reclassificationFeeTotal ?? 0)}（JOIN ${(p.totalConsumption as any).reclassificationJoinStakeTotal} + 反对 ${(p.totalConsumption as any).reclassificationStakeTotal}）` : null,
        ...
        <div style={{ border: "1px solid #444", borderRadius: 6, padding: 8, display: "flex", flexDirection: "column", gap: 6 }}>
          ...
          {(() => {
            ...
            const placeholderText = isCustomSubType ? "输入自定义理由（最长20字）"
              ...
              : textAreaDisabled
              ? (p.draftHasRelationTarget && p.relationType === "correct" ? "关系消息不能作为更正目标，请选择普通文本消息" : p.isTagWithQuickAnnotate ? "已选择附加关系，此处不可输入" : p.isMergeType ? "归并关系为与会者-消息关系，此处不应输入内容" : p.isGovernanceOrOpsType && p.relationType === "proposal"
